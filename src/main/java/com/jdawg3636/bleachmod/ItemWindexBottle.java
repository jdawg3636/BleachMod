...
public class ItemWindexBottle extends Item {
    ...
    public ItemStack finishUsingItem(ItemStack stack, Level worldIn, LivingEntity entityLiving) {
        ...
        if (entityLiving instanceof Player entityPlayer) {
            // Give Empty Bottle
            entityPlayer.getInventory().add(Reference.ITEM_EMPTY_WINDEX_BOTTLE.get().getDefaultInstance());
            // Inflict Damage
            if (!worldIn.isClientSide()) entityPlayer.hurt(Reference.DAMAGE_SOURCE_WINDEX, 600.0F);
        }
